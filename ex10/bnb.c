...
void branch_and_bound(int *final_sol)
{
  ...
  /* YOUR CODE GOES HERE */
  struc_sol *current_elem = (struc_sol *)malloc(sizeof(struc_sol));
  current_elem->fixed = 0;
  frac_bound(current_elem, current_elem->fixed);
  double current_best = current_elem->val;
  insert(*current_elem);
  struc_sol *child_1 = (struc_sol *)malloc(sizeof(struc_sol));
  struc_sol *child_2 = (struc_sol *)malloc(sizeof(struc_sol));
  child_1->fixed = 0;
  child_2->fixed = 0;

  struc_sol current_max;

  while(QueueSize > 0 && current_max.bound > current_best)
  {
     current_max = removeMax();
     child_1->fixed= current_max.fixed +1;
     copy_array(current_max.solution_vec, child_1->solution_vec);
     child_1->solution_vec[child_1->fixed] = 1;


     child_2->fixed = current_max.fixed +1;
     copy_array(current_max.solution_vec, child_2->solution_vec);

     child_2->solution_vec[child_2->fixed] = 0;

     int total_weight1 = 0;
     int total_weight2 = 0;
     for(int i = 1; i<= child_1->fixed; i++)
     {
       if(child_1->solution_vec[i]==1)
           total_weight1+=item_weights[temp_indexes[i]];

       if(child_2->solution_vec[i]==1)
           total_weight2+=item_weights[temp_indexes[i]];
     }

     if(total_weight1 <= Capacity)
     {
       frac_bound(child_1, child_1->fixed);
       if(child_1->val > current_best)
       {
         current_best = child_1->val;
         copy_array(child_1->solution_vec, final_sol);
         current_max.bound = child_1->bound;
       }
       insert(*child_1);
     }
     ...
     {
       frac_bound(child_2, child_2->fixed);
       if(child_2->val > current_best)
       {
         current_best = child_2->val;
         copy_array(child_2->solution_vec, final_sol);
         current_max.bound = child_2->bound;
